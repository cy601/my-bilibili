...
import com.TanNgee.bilibili.dao.FileDao;
...
public class FileService {
    ...
    public String uploadFileBySlices(MultipartFile slice,
        ...
        File dbFileMD5 = fileDao.getFileByMD5(fileMD5);  //通过MD5，判断文件是否已经上传过
        if (dbFileMD5 != null) {
            return dbFileMD5.getUrl();  //存在，直接返回，实现妙传
        }
        String url = fastDFSUtil.uploadFileBySlices(slice, fileMD5, sliceNo, totalSliceNo); //保存到fastDFS
        if (!StringUtil.isNullOrEmpty(url)) {
            ...
            fileDao.addFile(dbFileMD5);  //保存到数据库
        }
        return url;
    }
